...
public class JavaScriptExecutionStrategy(ILogger<JavaScriptExecutionStrategy> logger) : ILanguageExecutionStrategy
{
    private readonly ILogger<JavaScriptExecutionStrategy> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public CreateContainerParameters CreateContainerConfig(string containerName, string executionDirectory, string language)
    {
        ...
        {
            Image = DockerImages.JavaScriptRunner,
            Name = containerName,
        ...
        long executionTimeMs, string executionDirectory)
    {
        ...
        _logger.LogInformation("JavaScript code execution complete. Success: {Success}, ExitCode: {ExitCode}, ExecutionTime: {ExecutionTime}ms",
