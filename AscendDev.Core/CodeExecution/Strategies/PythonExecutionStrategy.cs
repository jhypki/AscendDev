...
public class PythonExecutionStrategy(ILogger<PythonExecutionStrategy> logger) : ILanguageExecutionStrategy
{
    private readonly ILogger<PythonExecutionStrategy> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public CreateContainerParameters CreateContainerConfig(string containerName, string executionDirectory, string language)
    {
        ...
        {
            Image = DockerImages.PythonRunner,
            Name = containerName,
        ...
        long executionTimeMs, string executionDirectory)
    {
        ...
        _logger.LogInformation("Python code execution complete. Success: {Success}, ExitCode: {ExitCode}, ExecutionTime: {ExecutionTime}ms",
